...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        material->Get(AI_MATKEY_REFRACTI, mat.refracti);

        // 1. ambient map
        loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_ambient", textures);
        // 2. diffuse map
    ...
    {
        if (!mat->GetTextureCount(type)) return;
        aiString str;
        mat->GetTexture(type, 0, &str); // just load the first texture
